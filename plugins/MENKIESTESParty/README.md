# MENKIESTESParty v2.1.0

Modular Paper Party/Guild framework by **CADERA** for Paper 1.21.11 / Java 21.

MENKIESTESParty remains a standalone public plugin. YAML is still the default lightweight backend; SQLite/MySQL and developer integrations remain optional. v2.1.0 adds an optional CdrJobs Public API v2 bridge without making CdrJobs mandatory.
...
- optional CdrJobs Five Paths profession integration
...
## CdrJobs Five Paths integration

When CdrJobs v1.6.0+ is installed, MENKIESTESParty can consume its Public API v2 and accepted `ProfessionActionEvent` stream. Daily/Weekly Party activity then inherits CdrJobs anti-exploit validation and the legacy raw activity listeners are suspended by default to prevent double-counting.

The bridge covers Miner, Farmer, Hunter, Lumberjack and Fisher. It also adds Party profession roster diagnostics, optional distinct-member **Five Paths Convergence**, and CdrJobs-specific `cdr_*` Party Projects.
...
CdrJobs is a soft dependency. Removing or disabling it does not disable MENKIESTESParty; the plugin returns to standalone fallback tracking.

See [`CDRJOBS_INTEGRATION.md`](CDRJOBS_INTEGRATION.md).

## Architecture v2

v2.0.0 introduced a clean architecture boundary without turning a normal single-server installation into a distributed system.
...
Existing Party/social/admin/progression data paths remain in place. SQLite/MySQL table layout is not rewritten by v2.1.0.
...
Architecture v2 ships `LOCAL` transport by default. It emits Party revision envelopes from the existing post-state event stream and performs no external network I/O.

There is **no Redis dependency, no network polling loop and no cross-server state replication claim**. The transport boundary exists so a future distributed module can be added without rewriting Party gameplay managers.
...
API v2 adds runtime architecture information, Social Profile snapshots, richer Member snapshots and Party revision values. API v1 remains registered for existing integrations. The CdrJobs bridge does not change either Party API version.
...
Existing placeholders remain. Architecture values include:
...
- CdrJobs bridge: optional, API v2 minimum when present
- CdrJobs authoritative Daily/Weekly activity: enabled while bridge is healthy
- Five Paths Convergence: distinct specialists, +10% CdrJobs Project progress by default
- Social profiles: enabled
- Dynamic Identity: automatic
- Dangerous admin actions: exactly-once token confirmation
- Folia: experimental only; not production-certified
...
GitHub Actions verifies Java 21 compilation/tests, API v1/v2 contracts, storage/admin/social/architecture tests, production JAR contents, MySQL integration and the repository's runtime compatibility gates.

## Documentation

- [`CHANGELOG.md`](CHANGELOG.md)
- [`RELEASE_NOTES_v2.1.0.md`](RELEASE_NOTES_v2.1.0.md)
- [`CDRJOBS_INTEGRATION.md`](CDRJOBS_INTEGRATION.md)
